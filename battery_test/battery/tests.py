...
class ProcessoModelTest(TestCase):
    def test_criar_processo(self):
        processo = Processo.objects.create(
            numero="12345", classificacao="DESENVOLVIMENTO"
        )
...
class BateriaModelTest(TestCase):
    ...
    def test_criar_bateria(self):
        bateria = Bateria.objects.create(
            processo=self.processo, numero_serie="ABC123", fabricante="Fabricante Teste"
        )
        self.assertEqual(
            str(bateria), f"Bateria ABC123 - Processo {self.processo.numero}"
        )
        self.assertTrue(bateria.codigo)

    def test_gerar_codigo_unico(self):
        bateria1 = Bateria.objects.create(
            processo=self.processo, numero_serie="ABC123", fabricante="Fabricante Teste"
        )
        bateria2 = Bateria.objects.create(
            processo=self.processo, numero_serie="DEF456", fabricante="Outro Fabricante"
        )
...
class DadoColetadoModelTest(TestCase):
    ...
    def test_criar_dado_coletado(self):
        dado = DadoColetado.objects.create(
            bateria=self.bateria, capacidade=80.0, ciclo=10, voltagem=12.6, corrente=1.2
        )
...
class ViewsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.processo = Processo.objects.create(
            numero="123", classificacao="DESENVOLVIMENTO"
        )
        self.bateria = Bateria.objects.create(
            processo=self.processo, numero_serie="XYZ123", fabricante="ABC"
        )
